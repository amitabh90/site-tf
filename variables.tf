...
locals {
  ...
  mysql_flexible_server_name_effective = var.mysql_flexible_server_name != "" ? var.mysql_flexible_server_name : "${var.name_prefix}-mysqlserver01-${var.environment}"
...
variable "app_insights_name" {
  type        = string
  description = "Explicit Application Insights component name (leave blank to auto-generate)."
  default     = ""
}

variable "log_analytics_workspace_name" {
  type        = string
  description = "Explicit Log Analytics workspace name (leave blank to auto-generate)."
  default     = ""
}

variable "action_group_name" {
  type        = string
  description = "Explicit Action Group name (leave blank to auto-generate)."
  default     = ""
}
...
variable "alert_email_receivers" {
  ...
  description = "List of email receivers for action group."
  default = [
    { name = "primary", email = "Amitabh.d@alifconsulting.com" }
  ]
}

variable "webapp_memory_threshold_mb" {
  type        = number
  description = "Alert when Average MemoryWorkingSet exceeds this many MB"
  default     = 1024
}

variable "webapp_response_time_threshold_ms" {
  type        = number
  description = "Alert when AverageResponseTime exceeds this many ms"
  default     = 1000
}

variable "storage_used_capacity_threshold_bytes" {
  type        = number
  description = "Alert when UsedCapacity exceeds this many bytes"
  default     = 800 * 1024 * 1024 * 1024
}

locals {
  app_insights_name_effective            = var.app_insights_name != "" ? var.app_insights_name : "${var.name_prefix}-appinsights-${var.environment}"
  log_analytics_workspace_name_effective = var.log_analytics_workspace_name != "" ? var.log_analytics_workspace_name : "${var.name_prefix}-loganalytics-${var.environment}"
  action_group_name_effective            = var.action_group_name != "" ? var.action_group_name : "${var.name_prefix}-action-group-${var.environment}"
}
